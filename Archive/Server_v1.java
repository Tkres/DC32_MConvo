...
public class Server_v1 extends PApplet {
			...
			if (!dialogueDone) {
				if (currentSpeechTimer.hasStopped()) {
					...
					if (curLine < lines.length) {
						...
						// wait
						currentSpeechTimer = new Countdown(remainingDurationOfTTS(voices[botid], 250, lines[curLine]));
					} else {
						dialogueDone = true;
					}
				}
	...
	public void keyPressed() {
		...
		if (key == 'd') {
			...
			if (targetLocations.size() > 0) {
				...
				if (printerControlOn)
					printer.sendMessage("Dialogue Begun.");

				String[] voices = { TTS.VICKI, TTS.BRUCE, TTS.TRINOIDS,
						TTS.BELLS, TTS.ZARVOX, TTS.HYSTERICAL };
				...
				for (int i = 0; i < lines.length; i++) {
					...
					// TODO: THE BELOW TRAY CMDS have to be timed and be done
					// synchronously with this talking text.
					println("---------------");
					for (int h = 0; h < bots.size(); h++) {
						if (h == botid) {
							commandTray("open", bots.get(botid));
					...
					int turntableMsg = (botid % 4) + 1;
					if (turntableControlOn)
						turntable.sendMessage(turntableMsg + "");
					delay(remainingDurationOfTTS(voices[botid], 250, lines[i]));

					// TODO: note: should also delay a minimum of the time
					// required to turn computers. (if on)

					// have to compensate for visualisation lag too.

					if (!dialogueOn) {
						println("Dialogue stopped.");
						break;
					}
				}

			} else {
				println("Dialogue can't be started with only "
						+ targetLocations.size() + " clients.");
			}
		}
